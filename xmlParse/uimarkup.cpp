...
XmlMarkupNode::XmlMarkupNode(XmlMarkup* pOwner, int iPos) : m_pOwner(pOwner), m_iPos(iPos), m_nAttributes(0)
{
}

XmlMarkupNode XmlMarkupNode::GetSibling()
{
    if( m_pOwner == NULL ) 
		return XmlMarkupNode();
...
bool XmlMarkupNode::HasSiblings() const
{
    if( m_pOwner == NULL ) 
		return false;
...
XmlMarkupNode XmlMarkupNode::GetChild()
{
    if( m_pOwner == NULL ) 
		return XmlMarkupNode();
...
bool XmlMarkupNode::HasChildren() const
{
    if( m_pOwner == NULL ) 
		return false;
...
XmlMarkupNode XmlMarkupNode::GetParent()
{
    if( m_pOwner == NULL ) 
		return XmlMarkupNode();
...
bool XmlMarkupNode::IsValid() const
{
    return m_pOwner != NULL;
}

LPCTSTR XmlMarkupNode::GetName() const
{
    if( m_pOwner == NULL ) 
		return NULL;
...
LPCTSTR XmlMarkupNode::GetAttributeValue(int iIndex)
{
    if( m_pOwner == NULL )
		return NULL;
    if( m_nAttributes == 0 ) 
...
LPCTSTR XmlMarkupNode::GetAttributeValue(LPCTSTR pstrName)
{
    if( m_pOwner == NULL )
		return NULL;
    if( m_nAttributes == 0 ) 
...
bool XmlMarkupNode::GetAttributeValue(int iIndex, LPTSTR pstrValue, SIZE_T cchMax)
{
    if( m_pOwner == NULL ) 
		return false;
    if( m_nAttributes == 0 ) 
...
bool XmlMarkupNode::GetAttributeValue(LPCTSTR pstrName, LPTSTR pstrValue, SIZE_T cchMax)
{
    if( m_pOwner == NULL ) 
		return false;
    if( m_nAttributes == 0 )
...
int XmlMarkupNode::GetAttributeCount()
{
    if( m_pOwner == NULL ) 
		return 0;
    if( m_nAttributes == 0 ) 
...
bool XmlMarkupNode::HasAttributes()
{
    if( m_pOwner == NULL ) 
		return false;
    if( m_nAttributes == 0 ) 
...
bool XmlMarkupNode::HasAttribute(LPCTSTR pstrName)
{
    if( m_pOwner == NULL ) 
		return false;
    if( m_nAttributes == 0 ) 
...
bool XmlMarkup::IsValid() const
{
    return m_pElements != NULL;
}

void XmlMarkup::SetPreserveWhitespace(bool bPreserve)
{
    m_bPreserveWhitespace = bPreserve;
}

bool XmlMarkup::Load(LPCTSTR pstrXML)
{
    Release();
...
bool XmlMarkup::LoadFromMem(BYTE* pByte, DWORD dwSize, int encoding)
{
#ifdef _UNICODE
...
bool XmlMarkup::LoadFromFile(LPCTSTR pstrFilename, int encoding)
{
    Release();
	LPCTSTR sFile = pstrFilename;
...
void XmlMarkup::GetLastErrorMessage(LPTSTR pstrMessage, SIZE_T cchMax) const
{
    _tcsncpy(pstrMessage, m_szErrorMsg, cchMax);
}

void XmlMarkup::GetLastErrorLocation(LPTSTR pstrSource, SIZE_T cchMax) const
{
    _tcsncpy(pstrSource, m_szErrorXML, cchMax);
}

XmlMarkupNode XmlMarkup::GetRoot()
{
    if(m_nElements == 0)
		return XmlMarkupNode();

    return XmlMarkupNode(this, 1);
}
...
bool XmlMarkup::_Parse(LPTSTR& pstrText, ULONG iParent)
{
...
XmlMarkup::XMLELEMENT* XmlMarkup::_ReserveElement()
{
...
void XmlMarkup::_SkipWhitespace(LPCTSTR& pstr) const
{
    while( *pstr > _T('\0') && *pstr <= _T(' ') )
		pstr = ::CharNext(pstr);
}

void XmlMarkup::_SkipWhitespace(LPTSTR& pstr) const
{
    while( *pstr > _T('\0') && *pstr <= _T(' ') )
		pstr = ::CharNext(pstr);
}

void XmlMarkup::_SkipIdentifier(LPCTSTR& pstr) const
{
    // 属性只能用英文，所以这样处理没有问题
...
void XmlMarkup::_SkipIdentifier(LPTSTR& pstr) const
{
    // 属性只能用英文，所以这样处理没有问题
    while( *pstr != _T('\0') && (*pstr == _T('_') || *pstr == _T(':') || _istalnum(*pstr)) )
		pstr = ::CharNext(pstr);
...
bool XmlMarkup::_ParseAttributes(LPTSTR& pstrText)
{   
    if( *pstrText == _T('>') ) 
		return true;
    *pstrText++ = _T('\0');
...
bool XmlMarkup::_ParseData(LPTSTR& pstrText, LPTSTR& pstrDest, char cEnd)
{
...
void XmlMarkup::_ParseMetaChar(LPTSTR& pstrText, LPTSTR& pstrDest)
{
...
bool XmlMarkup::_Failed(LPCTSTR pstrError, LPCTSTR pstrLocation)
{
    // Register last error
